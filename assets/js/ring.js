      ...
      response.map((stackId,plan) => {
        if (stackId != 0) {
          ...
          contract.methods.getInfoStack(plan,stackId).call({from:currentAddr}).then(infoStack => {
            ...
            let optionPlan = $(`<option data-stack-plan=${plan} data-stack-${stackId}=${infoStack[3][infoStack[7]-1]}>Plan ${plan}</option>`).attr('value',stackId);
      ...
      if(!response[i]) {
        ...
        $(`#price [data-plan=${index}]`).on('click', function(){
          ...
          if(contract) {
            ...
            let referralStackId = $.urlParam('stack');
            referralCode = referralCode != null ? referralCode : 0;
            referralStackId = referralStackId != null ? referralStackId : 0;
            contract.methods.invest(plan,referralCode,referralStackId).send({
              from: currentAddr,
...
$('#about .container .top-margin button:contains(withdraw)').click(function(){
  ...
  let investmentId = $(this).prev().find('option:selected').attr(`data-stack-${stackId}`);
  if(contract) {
    contract.methods.withdraw(stackId,investmentId).send({from:currentAddr}).then(() => {
      iziToast.success({
        title: 'OK',
        message: "W",
